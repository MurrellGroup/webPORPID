...
| Interactive UMI cutoffs | A changed cutoff required rerunning the model externally | Reclassifies the complete fitted family table and re-encodes it directly for consensus workers | Posterior fitting is unchanged; only the accepted decision thresholds change. Display thinning is never used for classification. |
| Interactive plot preparation | Repeated per-sample scans would scale as samples × families | One linear bucketing pass builds the UMI and consensus checkpoint inputs | Exact frequency tables and decisions are unchanged. |
