...
- The functional filter is evaluated only on exact collapsed nucleotide variants; uncollapsed UMI-family consensuses never receive functional decisions. It selects the longest complete start-to-stop ORF across three frames and rejects a sequence as `noORF-reject` when none exists. Eligible sample ORFs are translated and amino-acid aligned first. Every aligned row must preserve its exact input residues: benign alphabet normalization is restored onto the returned gap path, while a changed residue count triggers a deterministic anchor/profile fallback that cannot delete or invent residues. The reference is then affine-aligned to that fixed sample profile, inserting only shared all-gap columns into existing rows, and the result is backtranslated with complete codon gaps. The supplied reference FASTA name is retained on the first row and the alignment is clipped to its first/last non-gap nucleotide. Passing variants use `sample_vN_K rm=0.yy`, retaining the family-count suffix. An unrecoverable functional-alignment error is stored against only that sample; its functional outputs are omitted while collapse, other samples, and later stages continue.
...
If that option was enabled for the run, the completed results page can reconstruct the same dialogue from stored consensus and UMI-family records. Accepting revised thresholds leaves UMI classification, consensus sequences, and contamination calls unchanged. If no global threshold changed, the before/after effective threshold tuple is compared per sample and only changed samples are recomputed; unchanged sample records, alignments, trees, and active edits are preserved. A global threshold change recomputes all samples. Only affected active edits are detached, audit history is retained, computation proceeds only as far as those stages had previously completed, and a deliberately deferred or skipped stage remains deferred or skipped.

## Browser execution and background retention

Pipeline computation runs in dedicated workers and requests a Screen Wake Lock while visible. **Run when not in focus** is checked by default for both the initial analysis and a post-consensus-review rerun. It starts a genuinely audible, very quiet Web Audio oscillator from the user's gesture and keeps it alive only for the active computation. This can reduce browser background throttling but cannot override browser or operating-system suspension. The CLI is the reliable route for unattended processing.
