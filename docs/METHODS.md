...
- The functional filter is evaluated only on exact collapsed nucleotide variants; uncollapsed UMI-family consensuses never receive functional decisions. It finds candidate coding regions, translates them, performs one joint amino-acid MSA with the reference, and projects alignment gaps back as complete codon triplets. It then applies start/stop, ambiguity, and reference-match gates and stores trimmed nucleotide/amino-acid outputs under the unchanged abundance-ranked collapsed names. Functional-pass alignment rows are additionally sliced at the first and last non-gap reference nucleotide, so query-only terminal sequence is excluded from FASTA exports and tree inference.
