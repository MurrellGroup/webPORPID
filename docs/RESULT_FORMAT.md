...
- **Collapse membership:** abundance-ranked `sample_vN_K` representative, stable member identifiers, a multiplicity that counts UMI families rather than reads, and collapsed-only functional decision/trimmed outputs. Functional calls may store a two-decimal `referenceMatch`; passing functional headers are `sample_vN_K rm=0.yy`, preserving the count suffix. Minimum agreement remains exclusively attached to the member UMI families.
- **Functional-filter errors:** optional sample-to-message entries identify a sample whose functional alignment could not be recovered. That sample has no partial functional decisions or functional alignment, while its collapse output and all other samples remain valid.
...
- **Interactive threshold decisions:** each accepted consensus-filter checkpoint identifier, acceptance time, global and sample-specific values, and a human-readable change list. The final values are also reflected in the stored effective configuration. Results can append a later decision and rerun only previously completed downstream stages from stored consensus calls; when global thresholds are unchanged, only samples whose effective thresholds changed are recomputed. Human review pauses are excluded from computational stage and total timing. Legacy UMI-phase records remain readable but new runs do not create them.
...
| `trimmed-nt-fasta` | Functionally accepted collapsed variants as trimmed nucleotide sequences labelled `sample_vN_K rm=0.yy` |
| `trimmed-aa-fasta` | Functionally accepted collapsed variants as translated amino-acid sequences labelled `sample_vN_K rm=0.yy` |
...
| `functional-nucleotide-alignment` | Functional reference as row one, then every passing `sample_vN_K rm=0.yy` variant, clipped to the reference endpoints |
