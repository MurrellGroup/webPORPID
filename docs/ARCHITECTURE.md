...
- An optional external-scratch backend writes each partition once through a long-lived asynchronous stream into a temporary subdirectory on a user-selected disk. This bypasses browser-origin quota and is the recommended browser mode when `maxReadsPerSample` is zero. Writers are closed before the count pass, partitions are then read as contiguous files, and the temporary subdirectory is removed after consensus or controlled failure. A tab/browser crash can leave the clearly prefixed `webporpid-scratch-*` directory behind for manual removal.
