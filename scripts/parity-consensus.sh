...
"${CXX:-c++}" -std=c++20 -O3 -I"$repo_dir/wasm/include" \
  "$repo_dir/tests/cpp/consensus_parity_driver.cpp" \
  ...
  -o "$work_dir/consensus-parity"
"$work_dir/consensus-parity" > "$work_dir/native.tsv"
(cd "$julia_project" && "$julia_bin" --startup-file=no --project=. "$repo_dir/tests/julia/consensus_parity.jl") > "$work_dir/julia.tsv"
node "$repo_dir/scripts/compare-consensus-parity.mjs" "$work_dir/native.tsv" "$work_dir/julia.tsv"
