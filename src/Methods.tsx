...
function PreprocessingPage() {
  ...
  <Section id="umi-offspring" kicker="Molecular grouping" title="Sparse two-edit UMI offspring model and LDA update" lead="Observed strings are classified as candidate real UMIs or likely error-derived offspring before family consensus is attempted.">
    ...
    <p>With interactive filtering enabled, model fitting pauses here. Exact family-size frequencies, binned posterior counts, and a deterministic display-only point sample are sent to the decision dialog. Sliders and unrestricted direct numeric entry update the visual guides; the accepted probability and family-size thresholds are then applied to every stored family without refitting the model. The original values, accepted values, time, and per-sample overrides are written to the project audit record and run log. Human review time is excluded from computational timing.</p>
...
function DownstreamPage() {
  ...
  <Section id="contamination" kicker="Run-wide filter" title="DP-means six-mer databases and self/non-self decision rule" lead="Each sequence is compared with run-derived sample signatures and the supplied external contamination panel.">
    ...
    <p>For a query, the closest non-self primary entry is reported only when its distance is strictly below <code>contaminationDistanceThreshold</code>. It is discarded only when no self primary entry lies at or below that threshold. By default self means the same sample. When a nonblank <code>donor_ID</code> is configured, every sample with that same donor ID belongs to one self group, so authentic overlap between those samples is not called contamination. External-panel entries are always non-self. If there is no primary call, the wider suspect database can produce a reported-but-retained suspect call. Exactly one decision per family is stored.</p>
    <h3>Performance-preserving implementation</h3><p>IUPAC resolution and six-mer counting are fused; only observed bins are sorted and materialized. Unambiguous signatures are reused between clustering and classification. Typed assignment buffers replace string snapshots. Exact inverted six-mer posting indexes accumulate every candidate dot product by visiting only shared bins; the same index is rebuilt in bounded increments as DP-means centers grow. A temporary-posting cap bounds memory and falls back to the sparse exact scan on an exceptionally fragmented run. A reverse-triangle norm bound also skips a fallback candidate only when it cannot mathematically beat the current exact distance. Within the resulting three-pass database, entry order, first-tie behavior, strict inequalities, means, and labels are preserved. The explicit three-pass cap can stop before a later fixed point and is therefore a documented speed/behavior tradeoff. Cooperative yields are triggered by elapsed CPU time rather than every fixed handful of records, keeping Skip responsive without tens of thousands of timer callbacks.</p>
  ...
  <Section id="artefact-agreement" kicker="Post-processing gates" title="Family-abundance artefact cutoff and minimum agreement" lead="The two gates are independent and their rejection reasons can overlap.">
    ...
    <p>In an interactive run, a second checkpoint appears after all consensus agreement values have been calculated and before downstream analysis. Exact size frequencies, agreement bins, and a deterministic scatter sample support live adjustment of agreement, quantile, and artefact-fraction guides. Accepted values are persisted and all downstream decisions use them.</p>
  </Section>
  ...
  <Section id="alignment-phylogeny" kicker="Exploration" title="MSA, double-precision FastTree, rooting, and mutation mapping" lead="Collapsed, uncollapsed, and functional alignments are separate inspectable analysis variants.">
    ...
    <p>Configured donor IDs add a donor-level workbench. It can jointly realign either collapsed haplotypes or functional-filter passes from all samples assigned to one donor, infer a new tree, color tips by source sample, and retain exact family-count bubble areas for collapsed rows. Prefixing tip identifiers with sample identity prevents cross-sample name collisions.</p>
  </Section>
  ...
  <Section id="results-file" kicker="Persistence" title="Versioned project file, validation, and exports" lead="The project begins at consensus calls and excludes raw reads and temporary spool data.">
    <p>The <code>.webporpid</code> file contains configuration (including donor IDs), exact input-slot mappings, preprocessing/family/consensus statistics, consensus sequences, optional-stage states, contamination calls, post-processing records, alignments, trees, collapse membership, retained small reference sequences, accepted interactive-threshold audit records, edit copies, audit history, timings, and logs. It is schema-validated, MessagePack encoded, gzip compressed, and prefixed by a versioned magic header.</p>
