...
const OPTIONAL_STAGE_LABELS: Record<OptionalStageName, string> = {
  ...
  collapse: "Haplotype collapse", tree: "Collapsed phylogenies",
};
...
function OptionalStagePanel({ bundle, progress, onCompute, onSkip }: { bundle: ResultBundle; progress?: DeferredAnalysisProgress;
  ...
  return <section className="optional-stage-panel"><header><div><span className="section-kicker">Resumable analysis</span><h2>Optional stages after consensus</h2><p>Deferred and skipped work is recorded explicitly. “Compute through” runs every missing upstream stage required for that output.</p></div></header>
    <div className="optional-stage-grid">{OPTIONAL_STAGE_ORDER.map((stage, index) => {
      const status = stageStatus(bundle, stage), missing = OPTIONAL_STAGE_ORDER.slice(0, index).filter((candidate) => !stageCompleted(bundle, candidate));
      const busy = Boolean(progress), current = progress?.stage === stage;
      return <article className={`optional-stage ${status.state}`} key={stage}><span>{OPTIONAL_STAGE_LABELS[stage]}</span><strong>{status.state}</strong><p>{status.detail}</p>{status.state !== "completed" && <button type="button" disabled={busy} onClick={() => onCompute(stage)}>Compute through {OPTIONAL_STAGE_LABELS[stage].toLowerCase()}</button>}{missing.length > 0 && <small>Also computes: {missing.map((item) => OPTIONAL_STAGE_LABELS[item]).join(" → ")}</small>}{current && <div className="on-demand-progress"><progress max="100" value={Math.round(progress.fraction * 100)} /><span>{progress.detail}</span><button type="button" className="danger" onClick={onSkip}>Skip this step</button></div>}</article>;
    })}</div>
  </section>;
}
...
export function ResultsExplorer({ bundle, onSaveResults, onBundleChange }: { bundle: ResultBundle; onSaveResults(): void; onBundleChange(bundle: ResultBundle): void }) {
  ...
  const functionalTips = useMemo(() => uncollapsedMetadata(sampleRecords.filter((record) => record.functionalPass === true)), [sampleRecords]);
  const edited = Boolean(collapsed.edit || uncollapsed.edit || functional.edit);
  const contaminationDone = stageCompleted(bundle, "contamination"), postprocessingDone = stageCompleted(bundle, "postprocessing");
  ...
  async function computeOptionalStage(target: OptionalStageName) {
    ...
    setDeferredError(""); setDeferredProgress({ stage: target, fraction: 0, detail: `Preparing to compute through ${OPTIONAL_STAGE_LABELS[target].toLowerCase()}…` });
    try {
  ...
  function openAlivibe(variant: AlignmentVariant) {
    ...
    editorUrl.searchParams.set("swigBridge", String(ALIVIBE_BRIDGE_VERSION)); editorUrl.searchParams.set("source", ALIVIBE_SOURCE_REVISION.slice(0, 12)); editorUrl.searchParams.set("release", "0.3.6");
  ...
  function alignmentBlock(variant: AlignmentVariant) {
    ...
    const heading = variant === "collapsed" ? "Collapsed haplotypes" : variant === "uncollapsed" ? "Uncollapsed consensus sequences" : "All functional-filter passes";
    const description = variant === "collapsed"
      ...
      : variant === "uncollapsed" ? "Every retained UMI-family consensus is shown separately and can be colored by its family minimum agreement."
        : "Every sequence that passed the functional filter is shown in the joint codon-aware alignment, clipped exactly to the first and last nucleotide of the functional reference.";
    ...
    return <section className="phylogeny-block" key={variant}><header><div><span className="section-kicker">{kicker}</span><h3>{heading}</h3><p>{description}</p></div><div><button type="button" className={tree ? "" : "primary"} disabled={busy} onClick={() => void inferTree(variant)}>{busy ? "Running FastTree…" : tree ? "Recalculate tree" : "Infer tree"}</button></div></header>
      ...
      <AlignmentTreeViewer fasta={current.fasta} newick={tree} alphabet={alphabet} onAlphabetChange={setAlphabet} frameOffset={current.frameOffset} referenceSequence={reference} leafMetadata={metadata} collapsed={variant === "collapsed"} treeStale={stale} name={`${safeDatasetName(bundle.config.dataset)}-${safeDatasetName(sample)}-${variant}`} />
    </section>;
  }
  ...
  return <main className="results-page">
    ...
    {!allSamples && <>{tab === "overview" && <section className="result-section">
      ...
      <div className="statistics-grid"><DualStatsTable title="PORPID call statistics" description="Each observed UMI is counted once. The aggregate BPB-reject bucket contributes reads / CCS but not an artificial UMI count; family sizes expose heteroduplex and LDA impact at both molecule and read level." rows={porpidStats} />{postprocessingDone && <><DualStatsTable title="Consensus and post-processing filters" description="Consensus-filter rows report every matching rejection category; a family can match more than one row." rows={postprocStats} familyLabel="Consensus families" /><DualStatsTable title="Functional-filter results" description="All functional categories used by the reference-anchored coding filter are shown, including zero-count categories and sequences not evaluated after an upstream rejection." rows={functionalStats} familyLabel="Consensus families" /></>}</div>
    ...
    {tab === "alignment" && <section className="result-section"><div className="alignment-switch"><div><h2>Tree + alignment workbench</h2><p>Collapsed haplotypes are the default phylogeny. Amino acid is translated directly from each active nucleotide alignment.</p></div><div><button className={alphabet === "nt" ? "active" : ""} onClick={() => setAlphabet("nt")}>Nucleotide alignment</button><button className={alphabet === "aa" ? "active" : ""} onClick={() => setAlphabet("aa")}>Amino-acid alignment</button></div></div>
      ...
      {postprocessingDone && sampleConfig?.functionalReference && !functional.fasta && <div className="empty-state">No sequences passed the functional filter for this sample, so there is no functional-sequence alignment to display.</div>}
    </section>}
