export interface PanelFilterResult {
  sequences: string[];
  scores: number[];
}

export interface IndependentPanelProgress {
  completed: number;
  total: number;
}
...
const NEGATIVE = -1e30;
const LOG_QUARTER = Math.log(0.25);

function normalizeQuery(sequence: string) {
...
export function buildPanelProfile(panelRows: readonly string[]): PanelProfile {
  ...
  if (!width || normalized.some((row) => row.length !== width))
    throw new Error("The reference panel must be a non-empty rectangular alignment.");
  ...
  return { width, probabilities };
}

interface AlignmentMap {
  query: string;
  queryToPanel: Int32Array;
  touchedBand: boolean;
}
 ...
 */
function bandedProfileAlignment(profile: PanelProfile, rawQuery: string, band: number): AlignmentMap {
  ...
  if (!Number.isSafeInteger(cells) || cells > 768 * 1024 * 1024)
    throw new Error("The independent panel alignment exceeds the browser memory safety limit.");
  ...
  previousM.fill(NEGATIVE); previousX.fill(NEGATIVE); previousY.fill(NEGATIVE);
  starts[0] = 0; ends[0] = Math.min(queryLength, band);
  previousM[0] = 0;
  ...
  for (let row = 1; row <= panelLength; row++) {
    ...
    starts[row] = start; ends[row] = end;
    currentM.fill(NEGATIVE); currentX.fill(NEGATIVE); currentY.fill(NEGATIVE);
    if (start === 0) {
    ...
    for (let column = Math.max(1, start); column <= end; column++) {
      ...
      if (diagonal >= previousStart && diagonal <= previousEnd) {
        ...
        if (previousX[previousIndex] > bestM) { bestM = previousX[previousIndex]; fromM = 1; }
        if (previousY[previousIndex] > bestM) { bestM = previousY[previousIndex]; fromM = 2; }
        const code = query.charCodeAt(column - 1), probability = code < ASCII_SYMBOLS
          ? profile.probabilities[(row - 1) * ASCII_SYMBOLS + code] : 0;
      ...
      if (column >= previousStart && column <= previousEnd) {
        ...
        xContinues = extended > opened; currentX[index] = xContinues ? extended : opened;
      }
      let yContinues = false;
      if (column - 1 >= start) {
        const leftIndex = index - 1, opened = currentM[leftIndex] + GAP_OPEN, extended = currentY[leftIndex] + GAP_EXTEND;
        yContinues = extended > opened; currentY[index] = yContinues ? extended : opened;
      }
  ...
  if (previousX[endpoint] > maximum) { maximum = previousX[endpoint]; state = 1; }
  if (previousY[endpoint] > maximum) { maximum = previousY[endpoint]; state = 2; }
  if (maximum <= NEGATIVE / 2) return { query, queryToPanel: new Int32Array(queryLength).fill(-1), touchedBand: true };

  const queryToPanel = new Int32Array(queryLength); queryToPanel.fill(-1);
  let row = panelLength, column = queryLength, touchedBand = false;
  while (row > 0 || column > 0) {
    if (row === 0) { queryToPanel[--column] = -1; state = 2; continue; }
    if (column === 0) { row--; state = 1; continue; }
    const start = starts[row], end = ends[row];
    if (column < start || column > end) return { query, queryToPanel, touchedBand: true };
    if ((start > 0 && column <= start + 2) || (end < queryLength && column >= end - 2)) touchedBand = true;
    const packed = trace[row * stride + column - start];
    if (state === 0) {
      queryToPanel[column - 1] = row - 1; row--; column--; state = packed & 3;
    } else if (state === 1) {
      row--; state = packed & 4 ? 1 : 0;
    } else {
      queryToPanel[column - 1] = -1; column--; state = packed & 8 ? 2 : 0;
    }
  }
  return { query, queryToPanel, touchedBand };
}

function alignAdaptively(profile: PanelProfile, sequence: string): AlignmentMap {
...
function maximumSubarrayScore(profile: PanelProfile, query: string, mapping: Int32Array) {
  ...
  for (let index = 0; index < query.length; index++) {
    const panelColumn = mapping[index]; if (panelColumn < 0) continue;
    const code = query.charCodeAt(index), probability = code < ASCII_SYMBOLS
      ? profile.probabilities[panelColumn * ASCII_SYMBOLS + code] : 0;
    current += -Math.log(probability + 0.01) + LOG_QUARTER;
...
export function filterQueriesAgainstPanel(
  sequences: readonly string[], panelRows: readonly string[],
  onProgress?: (progress: IndependentPanelProgress) => void,
): PanelFilterResult {
  const profile = buildPanelProfile(panelRows), extracted: string[] = Array(sequences.length), scores = new Array<number>(sequences.length);
  for (let index = 0; index < sequences.length; index++) {
    ...
    for (let position = 0; position < aligned.queryToPanel.length; position++) if (aligned.queryToPanel[position] >= 0) {
      if (first < 0) first = position; last = position;
    }
    ...
    if ((index & 7) === 7 || index + 1 === sequences.length) onProgress?.({ completed: index + 1, total: sequences.length });
  }
  return { sequences: extracted, scores };
}
