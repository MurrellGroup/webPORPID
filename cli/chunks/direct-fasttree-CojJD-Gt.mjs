import { n as BinaryWriter } from "./binary-68r8u1WT.mjs";
...
function parseSamples(source) {
	return Object.entries(source).map(([name, raw]) => {
		...
		return {
			...
			functionalReference: text(pick(value, "ff_ref", "functionalReference"), `${name}.ff_ref`, true) || void 0,
			panelSequences: [],
...
const basename = (value) => normalizePath(value).split("/").at(-1);
...
function createDirectFastTreeRunner(javascriptPath, wasmPath) {
...
var FastTreeWorkerClient = class {
	constructor(worker) {
		this.worker = worker;
...
function createFastTreeRunner(javascriptPath, wasmPath, size = 1, workerPath = new URL("../porpid-fasttree-worker.mjs", import.meta.url)) {
	const count = Math.max(1, Math.floor(size));
	if (count === 1) {
		const direct = createDirectFastTreeRunner(javascriptPath, wasmPath);
		direct.close = async () => {};
		return direct;
	}
	const clients = Array.from({ length: count }, () => new FastTreeWorkerClient(new Worker(workerPath)));
	let cursor = 0;
	const run = (alignment) => clients[cursor++ % clients.length].call({
		javascriptPath,
		wasmPath,
		alignment
	});
...
//#endregion
export { resolveReferenceFiles as a, parseConfigYaml as i, createFastTreeRunner as n, resultConfig as o, compileConfig as r, createDirectFastTreeRunner as t };
